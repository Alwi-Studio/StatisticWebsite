...
## Staff activity MySQL dashboard

The preview dashboard reads player statistics through the server-side
`/api/statistics` Vercel function. Database credentials are only used by that
function and must never be added to browser-side JavaScript.

Copy the variable names from `preview/.env.example` into the Vercel project's
environment variables. Use a dedicated MySQL account with only `SELECT`
permission on the player statistics table.

The default expected schema is:
...
CREATE TABLE staffactivity (
  ...
  amount_chat BIGINT NOT NULL DEFAULT 0,
  day INT NOT NULL,
  month INT NOT NULL,
  year INT NOT NULL
);
...
If the existing table or columns use different names, configure
the `MYSQL_ACTIVITY_TABLE` and `MYSQL_*_COLUMN` variables.
Set `MYSQL_SSL=true` when the MySQL provider requires TLS connections.

For local development, `pnpm dev` serves the dashboard on port `3000` and the
statistics API on port `3001`. Restart the command after changing `preview/.env`.
